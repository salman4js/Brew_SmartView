...
import CustomCalculationHandler from "../../custom.config.handler/custom.calculation.handler";
...
class CheckOutView extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            billingInfo: {
                gstPrice: undefined, roomPrice: undefined, totalPrice: undefined, isNegativeValue: false
            },
            customModal: {
                show: false,
            ...
            printingDetails: {
                invoice: true, tInvoice: false, igst: undefined, cgst: true, gstin: undefined
            },
    ...
    _triggerAlertMessage(options) {
        var alertMessageOptions = {
            isCentered: false, isRestrictBody: true, header: options.message
        };
    ...
    templateHelpers() {
        if (this.state.isFetched) {
            ...
            var opts = {
                color: "black", marginTop: (this.state.height / 2.5) + "px", textCenter: true
            };
            return this._triggerActivityLoader(opts);
        }
    };

    // Put an activity loader on any view!
    _triggerActivityLoader(opts) {
        return activityLoader(opts);
    };

    getIsFormulaCustomizationEnabled(){
    ...
    isTodayCheckoutDate() {
        var todayDate = this.getTodayDate(), providedCheckoutDate = this.getProvidedCheckoutDate();
    ...
    prepareConfigOptions() {
        this.configOptions = {
            isExtraCalc: this.getExtraBedCalcConfig(), isAdvanceRestricted: this.getIsAdvanceRestricted()
        };
    };
    ...
    onCloseCustomModal() {
        this.setState(prevState => ({
            ...prevState, customModal: {
                ...prevState.customModal, show: false
            }
        }));
    ...
    errorAlertContainerData() {
        ...
        var data = [{status: 'error', message: misMatchCheckoutDate}, {
            status: 'error', message: "Please verify customer details before checkout."
        }];
        this.checkForRoomTransferAlert(data);
        return data;
    };
    ...
    renderAlertContainer() {
        if (this.state.isFetched) {
            ...
            return <PropertyAlertContainer data={getAlertContainerLang}
                                           viewRefHeight={(alertContainerHeight) => this._updateState((this.state.height - alertContainerHeight), 'height')}/>
        }
    };
    ...
    calculateStayedDays() {
        ...
        if (this.getIsHourly()) {
            return brewDate.diffHours(checkInDateTime, checkOutDateTime);
        } else {
    ...
    setBillingInformation() {
        this.setState(prevState => ({
            ...prevState, billingInfo: {
                ...
                totalPrice: this.getTotalPayableAmount(),
    ...
    getAmountForStayedDays() {
        var roomPrice = this.getRoomPrice(), stayedDays = this.calculateStayedDays();
        return Number(roomPrice) * Number(stayedDays);
    };

    // Calculate total amount!
    getTotalPayableAmount() {
        const amountForStayedDays = this.getAmountForStayedDays(),
    ...
    fetchCustomFormula() {
        ...
        if(!this.customizedFormula){
            function parseResults(results){
                const customFormula = {};
                results.result[0].fields.map((opts) => {
                    customFormula[opts.fieldName] = opts.fieldCustomFormula;
                });
                return customFormula;
            }
            CommonUtils.dispatchRequest({
                widgetName: 'customConfigCalc', accInfo: this.state.params.accIdAndName,
                method: 'get', query: {selectedConfigOnly: true}
            }).then((result) => {
    ...
    async fetchBillingDetails() {
        var params = {
            roomtype: this.state.data.roomModel.suiteName,
            stayeddays: this.state.stayeddays + ' Days',
            roomid: this.state.data.roomModel._id,
            isHourly: this.getIsHourly(),
            extraCalc: this.getIsExtraCalcEnabled()
        }
    ...
    prepareUserCheckoutDetails() {
        this.checkoutDetails = {
            userid: this.state.userModel._id,
            username: this.state.userModel.username,
            roomid: this.state.data.roomModel._id,
            stayeddays: this.state.stayeddays,
            checkoutdate: this.getTodayDate(),
            checkoutTime: getTimeDate().getTime,
            checkinDate: this.state.userModel.dateofcheckin,
            checkinTime: this.state.userModel.checkinTime,
            roomtype: this.state.data.roomModel.suiteName,
            prebook: this.state.data.roomModel.preBooked,
            amount: this.getTotalPayableAmount(),
            refund: this.getRefundAmount(),
            totalDishAmount: 0,
            isGst: this.getIsGSTEnabled(),
            foodGst: 0,
            stayGst: this.state.billingInfo.gstPrice,
            roomno: this.state.data.roomModel.roomno,
            dateTime: this.getDateTime(),
            checkoutBy: getCurrentUser()
        };
    ...
    async performCheckout() {
        // Form up the params!
        var data = this.checkoutDetails, modalData, result = await this.checkoutUtils.onCheckout(data);
        if (result.data.success) {
            this._toggleLoader(false);
            this._updatePropertyController({
                reloadSidepanel: {silent: true},
                persistStatusView: false,
                widgetTileModel: {
                    objectIdToBeUpdated: result.data.updatedModel._id,
                    ...
                    action: 'REMOVE',
                    keysToCompare: ['_id', 'room']
                }, // After clearing the data from the
                // widget tile model, clear the model from the room status model too to keep the data in sync.
                updateUserCollection: {id: result.data.updatedModel._id, action: 'CHECK-OUT'},
                updatedModel: result.data.updatedModel
            });
            modalData = {
            ...
            modalData = {
                header: checkoutViewConstants.checkoutFailureError,
                isCentered: false,
                isRestrictBody: true,
                isErrorToast: true,
    ...
    formPrintDetailsObj() {
        this.printDetails = {
            ...
            extraBeds: this.state.userModel.extraBeds, // amount: this.state.billingInfo.totalPrice,
    ...
    updatePrintingDetailsPref(pref) {
        ...
        this.setState(prevState => ({
            ...prevState, printingDetails: {
                ...prevState.printingDetails, invoice: pref.invoice, tInvoice: pref.tInvoice
            }
        }), () => {
            this.windowPrint();
        })
    ...
    _triggerEditCustomerDetails() {
        ...
        userModel['amountFor'] = undefined;
        userModel['dateTime'] = undefined;
        userModel['updatedAdvance'] = undefined;
        userModel['roomId'] = undefined;
        ...
        this.props.dashboardController({
            dashboardMode: propertyContainerConstants.DASHBOARD_MODE.propertyReadView,
            queryParams: [{key: 'selectedModel', value: this.state.userModel._id}, {
                key: 'isEditable', value: 'true'
            }, {key: 'method', value: 'edit-user-model'}, {key: 'uniqueId', value: 'userId'}, {
                key: 'clientModelKey', value: 'updatedUserModel'
            }, {key: 'serverModelKey', value: 'updatedUserModel'}],
            selectedRoomConstant: propertyContainerConstants.PROPERTY_VIEW.propertyUser,
    ...
    logDialogBodyItemView() {
        ...
        if (maintainanceLogType?.data) {
            return <MetadataFields data={this.state.maintainanceLog}
                                   updateData={(updatedData) => this._updateState(updatedData, this.state.maintainanceLog)}/>
        } else {
            this.fetchMaintainanceLogType();
            var opts = {
                color: "black", textCenter: true
            };
            return this._triggerActivityLoader(opts);
        }
    };
    ...
    onCheckout() {
        var data = {
            ...
            isFooterEnabled: true,
            footerButtons: [{
                btnId: 'Cancel', variant: 'secondary', onClick: this.onCloseCustomModal.bind(this)
            }, {
                btnId: 'Checkout', variant: 'success', onClick: this.handleCheckoutConfirmed.bind(this)
            }]
        };
        this.setCustomModal(data);
    };
    ...
    fetchHTMLContent() {
        var options = {
            ...
            accId: this.state.params.accIdAndName[0], templateName: checkoutViewConstants.constantKey
        };
