...
import { getRefundTrackerList, updateRefundPercentage } from "./refund.tracker.utils";
...
const RefundTracker = () => {
  ...
  // Current refund percentage!
  var currentRefundPercentage = getStorage('refundPercentage');
  
  // Refund percentage state handler!
  const [refundPercentage, setRefundPercentage] = useState(currentRefundPercentage);
  
  // Input field state handler for refund amount updater!
  const [refundInput, setRefundInput] = useState([
    {
      value: refundPercentage,
      placeholder: "Describe your refund percentage here!",
      label: "Update Refund Percentage",
      name: 'refundPercentage',
      attribute: 'textField',
      isRequired: true,
      inlineToast: {
        isShow: false,
        inlineMessage: 'Please enter a valid refund percentage'
      }
    }
  ])
  ...
  function _showHeaderChildView(){
    return(
      <div className = "brew-cursor" onClick = {() => _triggerRefundModal(true)}>
  ...
  // Trigger refund modal to set the refund amount!
  function _triggerRefundModal(action){
    setModalAssist(prevState => ({...prevState, triggerRefundModal: action}))
  }
  
  // Get the field data in the server understandable format!
  function getFieldData(state){
    return nodeConvertor(state);
  }
  
  // On update refund percentage!
  async function onUpdateRefundPercentage(){
    const isFieldValid = await validateFieldData(refundInput, setRefundInput);
    if(isFieldValid.length === 0){
      const fieldData = getFieldData(refundInput);
      const result = await updateRefundPercentage(splitedIds[0], fieldData);
      if(result.data.success){
        setStorage("refundPercentage", result.data.refundPercentage);
        setRefundPercentage(result.data.refundPercentage)
        _triggerRefundModal(false)
      }
    } 
  }
  
  // Custom modal child view!
  function customModalBodyItemView(val, setVal){
    return(
  ...
  function _showCustomModalDialog(onCloseAction, onCreateAction, val, setVal){
    // Form up the data for custom modal!
    const data = {
      show: true,
      onHide: onCloseAction,
      header: "Customize refund percentage",
      centered: true,
      modalSize: "medium",
      footerEnabled: true,
      footerButtons: [
    ...
    return(
      <CustomModal modalData = {data} showBodyItemView = {() => customModalBodyItemView(val, setVal)} />
    )
  }
  ...
  return(
    <div>
      ...
      {modalAssist.triggerRefundModal && (
        _showCustomModalDialog(_triggerRefundModal, onUpdateRefundPercentage, refundInput, setRefundInput)
      )}
